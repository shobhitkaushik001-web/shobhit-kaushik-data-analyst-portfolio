...
const f={id:document.getElementById('projectId'),title:document.getElementById('pTitle'),github:document.getElementById('pGithub'),description:document.getElementById('pDescription'),type:document.getElementById('pType'),tools:document.getElementById('pTools'),image:document.getElementById('pImage'),published:document.getElementById('pPublished')};
...
function editProject(p){document.getElementById('editorTitle').textContent='Edit Project';f.id.value=p.id;f.title.value=p.title;f.github.value=p.github_url;f.description.value=p.description;f.type.value=p.project_type||'Data Analytics';f.tools.value=(p.tools||[]).join(', ');f.published.checked=!!p.is_published;openEditor()}
...
form.onsubmit=async e=>{e.preventDefault();msg(editorMsg,'Saving...');try{let imageUrl=null;if(f.image.files[0])imageUrl=await uploadImage(f.image.files[0]);const payload={title:f.title.value.trim(),github_url:f.github.value.trim(),description:f.description.value.trim(),project_type:f.type.value,tools:f.tools.value.split(',').map(x=>x.trim()).filter(Boolean),is_published:f.published.checked,updated_at:new Date().toISOString()};if(!/^https:\/\/github\.com\//i.test(payload.github_url))throw new Error('Use a valid https://github.com/... repository URL.');if(imageUrl)payload.image_url=imageUrl;const result=f.id.value?await sb.from('projects').update(payload).eq('id',f.id.value):await sb.from('projects').insert(payload);if(result.error)throw result.error;msg(editorMsg,'Project saved.','success');setTimeout(async()=>{closeEditor();await loadProjects()},450)}catch(err){msg(editorMsg,err.message||'Could not save project.','error')}};
